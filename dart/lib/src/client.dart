...
class DatamaskingValidationException implements Exception {
  final List<String> errors;

  DatamaskingValidationException(this.errors);

  @override
  String toString() => 'DatamaskingValidationException: ${errors.join("; ")}';
}
...
/// Data Masking API client.
///
/// For more information, visit: https://apiverve.com/marketplace/datamasking?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [text] (required) - The text containing sensitive data to mask
/// * [types] - Array of data types to mask (default: all types)
class DatamaskingClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    ...
    'types': ValidationRule(type: 'array', required: false),
  };

  DatamaskingClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/datamasking',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DatamaskingValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DatamaskingValidationException(errors);
    }
  }

  /// Execute a request to the Data Masking API.
  ///
  /// Parameters are validated before sending the request.
  Future<DatamaskingResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DatamaskingException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DatamaskingResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DatamaskingException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DatamaskingException('Resource not found');
      } else {
        throw DatamaskingException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DatamaskingException) rethrow;
      throw DatamaskingException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Data Masking API client.
class DatamaskingException implements Exception {
  final String message;

  DatamaskingException(this.message);

  @override
  String toString() => 'DatamaskingException: $message';
}
